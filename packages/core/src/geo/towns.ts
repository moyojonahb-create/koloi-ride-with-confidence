/**
 * Multi-town configuration — VERBATIM PORT of `src/lib/towns.ts`.
 *
 * Copied without modification so the two copies stay diff-able. It needed no
 * changes: the module has no imports, no DOM globals and no `import.meta.env`,
 * which is unusual enough in this codebase to be worth stating.
 *
 * There is no DIVERGENCE entry because nothing diverges. If you change
 * behaviour here, either change it in `src/lib/towns.ts` too or open one — the
 * default rule in DIVERGENCE.md is fix in both or neither.
 *
 * The service areas, bounds and quick picks are operational data: they decide
 * where rides can be requested at all. Treat edits as a product change.
 *
 * ─────────────────────────── ported content below ───────────────────────────
 */
