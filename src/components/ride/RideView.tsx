...
export default function RideView() {
  ...
  // Handle quick pick
  const handleQuickPickSelect = (pick: {
    name: string;
    lat: number;
    lng: number;
  }) => {
  ...
  const handleRequestRide = async () => {
    ...
    if (!pickupLocation || !dropoffLocation || !fareEstimate) {
      toast({
        title: 'Select pickup and destination',
        variant: 'destructive'
      });
      return;
    }
  ...
  return <div className="min-h-screen bg-primary flex flex-col">
      ...
      {/* Status Banner */}
      {rideStatus !== 'idle' && <div className="px-4 pb-2">
      ...
      <main className="flex-1 flex flex-col pb-4 gap-4 px-[8px] py-0 my-0 mx-0 text-secondary-foreground">
        ...
        <div className="bg-background rounded-3xl shadow-koloi-xl pt-4 pb-6 px-5 flex flex-col">
          ...
          {activeField && <div className="space-y-4 animate-fade-in">
              ...
              <div className="bg-accent/10 rounded-2xl p-4 space-y-3">
                <div className="relative">
                  ...
                  <Input placeholder={activeField === 'pickup' ? 'Search pickup location...' : 'Search destination...'} value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="pl-12 h-14 bg-background border-0 rounded-xl text-base font-medium shadow-koloi-sm focus-visible:ring-accent" autoFocus />
                </div>

                {/* My Location Button (only for pickup) */}
                {activeField === 'pickup' && <button onClick={handleUseMyLocation} disabled={gpsState.status === 'loading'} className="w-full flex items-center gap-4 p-4 rounded-xl bg-background hover:bg-koloi-gray-100 transition-colors text-left">
                    <div className="w-10 h-10 rounded-full bg-accent flex items-center justify-center shrink-0">
                      {gpsState.status === 'loading' ? <Loader2 className="w-5 h-5 animate-spin text-accent-foreground" /> : <Crosshair className="w-5 h-5 text-accent-foreground" />}
                    </div>
                    <div>
          ...
          {!activeField && <div className="space-y-4">
              {/* Fare - Simple inline */}
              {fareEstimate && <div className="pt-2">
              ...
              <Button onClick={handleRequestRide} disabled={!canRequestRide} className={cn('w-full h-14 text-base font-bold rounded-2xl transition-all gap-2', canRequestRide ? 'bg-accent text-accent-foreground hover:brightness-105 shadow-koloi-md' : 'bg-koloi-gray-300 text-muted-foreground')}>
                {isRequesting ? <>
                    <Loader2 className="w-5 h-5 animate-spin" />
                    Finding drivers...
                  </> : !user ? 'Sign in to request' : canRequestRide ? <>
                    Request Ride
