...
const Header = ({ onLoginClick, onSignupClick, onFavoritesClick, onHistoryClick, transparent = false }: HeaderProps) => {
  ...
  return (
    <header className={`fixed top-0 left-0 right-0 z-50 ${transparent ? 'bg-transparent' : 'glass-nav'}`}>
      <div className="voyex-container">
        <nav className="flex items-center justify-between h-[60px] lg:h-[64px]">
          ...
          <div className="hidden lg:flex items-center gap-0.5 ml-8">
            {navItems.map((item) => (
              <a
                key={item.label}
                href={item.href}
                className="text-[15px] font-semibold text-foreground/70 hover:text-foreground hover:bg-muted/50 px-5 py-2.5 rounded-xl transition-all duration-150"
              >
                {item.label}
              </a>
            ))}
          ...
          <div className="hidden lg:flex items-center gap-2">
            ...
            <a
              href="#help"
              className="text-[15px] font-medium text-foreground/70 hover:text-foreground hover:bg-muted/50 px-4 py-2.5 rounded-xl transition-all duration-150"
            >
              Help
            </a>
            {!loading &&
          ...
          <button
            ...
            className="lg:hidden p-3 hover:bg-muted/50 text-foreground rounded-xl active:scale-90 transition-all"
